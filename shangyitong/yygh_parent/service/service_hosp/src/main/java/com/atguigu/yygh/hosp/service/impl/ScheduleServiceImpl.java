...
public class ScheduleServiceImpl extends ServiceImpl<ScheduleMapper, Schedule> implements ScheduleService {
    ...
    //select count(docCount) as docCount,sum(reservedNumber) as reservedNumber,sum(availableNumber) as availableNumber
    // from 表 where hoscode=? and decode=? group by workDate limit(?,?)desc
    public Map<String, Object> getRuleSchedule(long page, long limit, String hoscode, String depcode) {
        //1.根据医院编号 和科室编号查询
        //条件封装 where hoscode=? and depcode=?  (hoscode,depcode)
    ...
    public Map<String, Object> getBookingScheduleRule(Integer page, Integer limit, String hoscode, String depcode) {
        ...
        for (int i = 0, len = dataList.size(); i < len; i++) {
            ...
            //从scheduleVoMap集合中根据key日期获取value值
    ...
    @Override
    //根据排班日期获取排班数据
    public Schedule getSchedule(String schduleId) {
    ...
    private IPage getListData(Integer page, Integer limit, BookingRule bookingRule) {
        //获取当前放号时间 年 月 日 小时 分钟
        DateTime releaseTime = this.getDateTime(new Date(), bookingRule.getReleaseTime());
        //获取预约周期
        Integer cycle = bookingRule.getCycle();
        //如果当天放号时间(即截止时间)已经过去了，预约周期从后一天开始计算，周期加一
        if (releaseTime.isBeforeNow()) {
            //截止时间在当前预约时间之前（即过了今天的预约时间），只能往后推一天预约
            cycle += 1;
        }
        //获取可预约所有日期，最后一天显示即将放号
        List<Date> dateList = new ArrayList<>();
        for (int i = 0; i < cycle; i++) {
            DateTime curDateTime = new DateTime().plusDays(i);//往后加天数
            String dateString = curDateTime.toString("yyyy-MM-dd");
            dateList.add(new DateTime(dateString).toDate());
        }
        //因为预约周期是不同的，每页显示日期最多7天数据，超过7天分页
    ...
    public Schedule packSchedule(Schedule schedule) {
        //设置医院名称
