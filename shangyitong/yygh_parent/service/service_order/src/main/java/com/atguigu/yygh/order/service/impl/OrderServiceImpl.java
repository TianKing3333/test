...
public class OrderServiceImpl extends ServiceImpl<OrderMapper, OrderInfo> implements OrderService {
    ...
    public Long saveOrder(String scheduleId, Long patientId) {
        ...
        if (new DateTime(scheduleOrderVo.getStartTime()).isAfterNow()
            ...
            System.out.println(new DateTime(scheduleOrderVo.getStartTime()).isAfterNow());
            System.out.println(scheduleOrderVo.getEndTime());
            System.out.println(new DateTime(scheduleOrderVo.getEndTime()).isBeforeNow());
    ...
    public Boolean cancelOrder(Long orderId) {
        ...
        //根据医院接口返回的数据
        if (result.getInteger("code") != 200) {
            throw new YyghException(result.getString("messaage"), ResultCodeEnum.FAIL.getCode());
        } else {
            //判断当前订单是否可以取消
    ...
    public Map<String, Object> getCountMap(OrderCountQueryVo orderCountQueryVo) {
        ...
        //2.获取x需要数据，日期数据 list集合
        List<String> dateList = orderCountVoList.stream().map(OrderCountVo::getReserveDate).collect(Collectors.toList());

        //3.获取y需要数据，具体数量 list集合
    ...
    //加入orderStatusString，且在枚举类中调用方法将状态的数字转成其代表的含义
